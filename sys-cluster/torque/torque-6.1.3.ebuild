...
MY_COMMIT="34ff845417cc0348dfe0a61a491e5e0f4adf6ef4"
DESCRIPTION="Resource manager and queuing system based on OpenPBS"
HOMEPAGE="http://www.adaptivecomputing.com/products/open-source/torque"
SRC_URI="
	https://github.com/adaptivecomputing/torque/archive/${MY_COMMIT}.tar.gz -> ${P}.tar.gz
...
PATCHES=(
	...
	"${FILESDIR}"/${P}-bool-fix.patch
	"${FILESDIR}"/${P}-munge-size.patch
	"${FILESDIR}"/${P}-port-args.patch
	"${FILESDIR}"/${P}-system-jsoncpp.patch
	"${WORKDIR}"/${PN}-6.0.4-glibc-2.34-pthread.patch
)
