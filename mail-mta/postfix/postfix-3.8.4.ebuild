...
pkg_postinst() {
	...
	# call attention to the config changes recommended for SMTP smuggling
	if [ -f /etc/postfix/main.cf ] ; then
		if ! grep -q smtpd_forbid_bare_newline /etc/postfix/main.cf ; then
			ewarn
			ewarn "Postfix recommends enabling smtpd_forbid_bare_newline = yes"
			ewarn "in /etc/postfix/main.cf; run etc-update or see"
			ewarn "https://www.postfix.org/smtp-smuggling.html"
			ewarn
			ewarn "If you do not wish to enable it, add"
			ewarn "smtpd_forbid_bare_newline = no to silence this warning."
			ewarn
		fi
	fi
}
