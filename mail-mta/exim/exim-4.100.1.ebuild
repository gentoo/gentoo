...
IUSE="arc berkdb +dane dcc +dkim dlfunc dmarc dmarc-native +dmarc-opendmarc
	...
	mysql nis pam perl pkcs11 postgres +prdr proxy redis sasl selinux socks5
	spf sqlite srs +ssl syslog +tdb tcpd +tpda"
REQUIRED_USE="
	arc? ( dkim spf )
	dane? ( ssl !gnutls )
	dmarc? (
...
src_configure() {
	...
	if use dmarc; then
		if use dmarc-opendmarc; then
			...
			cat >> Makefile <<- EOC
				# Enable experimental dmarc (which needs PSL)
				LOOKUP_PSL=yes
...
pkg_postinst() {
	...
	einfo
	elog "Support for eximon via USE=X was dropped in 4.99 ebuilds,"
	elog "for eximon code no longer compiles using recent compilers."
}
