...
# eds-3.46.0[weather] and libsoup:3 because of build failure https://gitlab.gnome.org/GNOME/gnome-panel/-/issues/41
# eds build with USE="weather" ensures eds is built against libsoup:3.
# eds built with USE="-weather" could be build with libsoup:2.4 because it does not need libgweather-4.2.0
# This would result in a build failure in gnome-panel
RDEPEND="
	...
	>=gnome-extra/evolution-data-server-3.46.0[weather]
	net-libs/libsoup:3
"
DEPEND="${RDEPEND}
	x11-base/xorg-proto
...
src_configure() {
	...
	# libsystemd is used and check if there are new sections!
