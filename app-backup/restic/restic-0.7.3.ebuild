...
src_compile() {
	...
	GOPATH="${WORKDIR}/${P}:$(get_golibdir_gopath)" go build "${mygoargs[@]}" || die
}

src_test() {
	GOPATH="${WORKDIR}/${P}:$(get_golibdir_gopath)" go test -timeout 30m -v -work -x ${EGO_PN}/cmd/... ${EGO_PN}/internal/... || die
}
