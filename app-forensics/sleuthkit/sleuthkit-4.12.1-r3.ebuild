...
IUSE="aff doc ewf static-libs test +threads zlib"
RESTRICT="!test? ( test )"
...
DEPEND="
	...
	ewf? ( virtual/zlib:= )
	java? (
		>=dev-java/c3p0-0.9.5.5-r2:0
		>=dev-java/commons-lang-3.20.0-r1:0
		>=dev-java/commons-validator-1.10.0:0
		>=dev-java/gson-2.13.2:0
		>=dev-java/guava-33.5.0:0
		>=dev-java/jdbc-postgresql-42.7.11:0
...
BDEPEND="
	virtual/pkgconfig
	java? ( >=dev-java/ant-1.10.15-r1:0 )
...
src_compile() {
	...
	if use java; then
		...
		java-pkg_jar-from --into "${TSK_JAR_DIR}" commons-lang
		java-pkg_jar-from --build-only --into "${TSK_JAR_DIR}" commons-validator
		...
		java-pkg_jar-from --build-only --into "${TSK_JAR_DIR}" mchange-commons
