...
all_ruby_prepare() {
	...
	sed -i -e '3igem "minitest", "~> 6.0"; gem "minitest-mock"; require "dalli"' \
		-e '/bundler/ s:^:#:' test/helper.rb || die

	sed -i -e "s:/tmp:${T}:" test/utils/certificate_generator.rb || die

	# Checks for unpackaged gems
	rm test/test_gem_compatibility.rb || die

	# FIXME:
	sed -e "/'handles SSL error during read operations'/,/^      end/ s:^:#:" \
		-e "/'handles SSL error during pipelined get'/,/^      end/ s:^:#:" \
		-i test/integration/test_network.rb || die
}
