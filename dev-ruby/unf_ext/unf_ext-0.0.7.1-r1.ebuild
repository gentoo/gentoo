...
each_ruby_configure() {
    if tc-is-clang ; then
	  sed -i -e "s/have_library('stdc++')/have_library('c++')/" ext/unf_ext/extconf.rb	
	fi
...
each_ruby_compile() {
    if tc-is-clang ; then
	  emake -Cext/unf_ext CFLAGS="${CFLAGS} -fPIC" CXXFLAGS="${CXXFLAGS} -fPIC" archflag="${LDFLAGS}" V=1
	else
	  emake -Cext/unf_ext CFLAGS="${CFLAGS}" archflag="${LDFLAGS}" V=1
	fi
