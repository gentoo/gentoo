...
all_ruby_prepare() {
	...
	# noop yarn
	sed -e 's/yarn/true/' \
		-i test/isolation/abstract_unit.rb || die
	sed -e '/test_css_option_with_cssbundling_gem/,/^  end/ s:^:#:' \
		-e '/test_css_option_with_asset_pipeline_tailwind/,/^  end/ s:^:#:' \
		-e '/test_css_option_with_asset_pipeline_sass/,/^  end/ s:^:#:' \
	...
	sed -e '/def test_app_update$/,/^  end/ s:^:#:' \
		-e '/def test_app_update_create_new_framework_defaults/,/^  end/ s:^:#:' \
		...
		-e '/def test_app_update_does_not_create_rack_cors/,/^  end/ s:^:#:' \
		...
		-e '/def test_app_update_does_not_generate_assets_initializer_when_sprockets_and_propshaft_are_not_used/,/^  end/ s:^:#:' \
		-e '/def test_app_update_does_not_generate_bootsnap_contents_when_skip_bootsnap_is_given/,/^  end/ s:^:#:' \
		-e '/def test_app_update_does_not_generate_manifest_config_when_propshaft_is_used/,/^  end/ s:^:#:' \
		-e '/def test_app_update_does_not_remove_rack_cors_if_already_present/,/^  end/ s:^:#:' \
		-e '/def test_app_update_preserves_propshaft/,/^  end/ s:^:#:' \
	...
	sed -e '/test "db:create works when schema cache exists and database does not exist" do/,/^      end/ s:^:#:' \
		-e '/test "db:drop failure because bad permissions"/,/^        end/ s:^:#:' \
		-e '/test "db:prepare creates test database if it does not exist" do/,/^      end/ s:^:#:' \
		-e '/test "db:prepare loads schema when database exists but is empty"/,/^      end/ s:^:#:' \
	...
	sed -e '/test_devcontainer_mariadb_mysql/,/^  end/ s:^:#:' \
		-e '/test_devcontainer_mariadb/,/^  end/ s:^:#:' \
	...
	sed -e '/test_can_exclude_files_from_being_tested_via_rake_task_by_setting_DEFAULT_TEST_EXCLUDE_env_var/,/^    end/ s:^:#:' \
		-e '/test_fail_fast$/,/^    end/ s:^:#:' \
		-e '/test_fail_fast_in_parallel_with_processes/,/^    end/ s:^:#:' \
		...
		-i test/application/test_runner_test.rb || die
	sed -e '/"enable full backtraces on test failures"/,/^    end/ s:^:#:' \
		-i test/application/test_test.rb || die
	...
	sed -e '/test_app_update_silence_deprecate_message/,/^  end/ s:^:#:' \
		-i test/generators/app_generator_test.rb || die
	#sed -e '/"does not replace backtrace filter when using --backtrace option"/,/^  end/ s:^:#:' \
	#	-i test/minitest/rails_plugin_test.rb || die
	rm test/minitest/rails_plugin_test.rb || die
}
