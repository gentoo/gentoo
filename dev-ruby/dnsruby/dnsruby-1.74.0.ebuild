...
ruby_add_rdepend "
	>=dev-ruby/base64-0.2:*
	>=dev-ruby/logger-1.6.5:0 =dev-ruby/logger-1*
...
all_ruby_prepare() {
	sed -i -e '/logger/s/~> 1.6/~> 1/' dnsruby.gemspec || die
	sed -i -e "/[Cc]overall/d" Rakefile || die
	sed -i -e '/display/d' \
		-e '/Display/,/^}/d' test/spec_helper.rb || die
	# TestResolverConfig#test_one_good_ns_rest_bad contacts 1.1.1.1
	sed -i -e '/res_config/d' test/ts_offline.rb || die
}
