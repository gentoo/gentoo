...
MY_P="NuSMV-${PV}"
NUSMV_A="${MY_P}.tar.gz"
MINISAT_A="37dc6c67e2af26379d88ce349eb9c4c6160e8543.zip"
...
src_unpack() {
	unpack ${NUSMV_A}
	if use minisat; then
		cp "${DISTDIR}/${MINISAT_A}" "${WORKDIR}/${MY_P}/MiniSat" || die
	fi
	if use zchaff; then
...
src_prepare() {
	...
	sed -i -e '/\s*install( .*html /s:${PROJECT_BINARY_DIR}:share/nusmv/doc OPTIONAL:' "${S}"/doc/prog-man/CMakeLists.txt || die
