...
LICENSE="GPL-2 MIT ZLIB Unicode-3.0"
SLOT="0"
...
# Don't install Debian stuff
# Don't use pandoc (even if rresent) to generate pdf and epub docs
# Don't compress docs
PATCHES=("${FILESDIR}"/no-debian-0.patch
		 "${FILESDIR}"/no-pandoc-0.patch
		 "${FILESDIR}"/no-compress-0.patch)
...
src_prepare() {
	...
	rm_po() {
		...
		rm -f info/${PN}.${1}.md
		rm -f info/${PN}.${1}.html
		sed -e "\\|/${1}/|d" -i data/CMakeLists.txt
	}
