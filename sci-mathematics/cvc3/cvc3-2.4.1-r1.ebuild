...
src_compile() {
	...
	if use emacs ; then
		pushd "${S}/emacs" || die "Couldn't change directory to emacs"
		elisp-compile *.el || die "emacs elisp compile failed"
		popd >/dev/null || die "Failed to reverse directory change"
	fi
...
src_install() {
	...
	if use isabelle; then
		...
		cat <<- EOF >> "${S}/settings" || die "Failed to create Isabelle configuration for CVC3"
...
pkg_postinst() {
	...
	if use isabelle; then
		if [[ -f "${EROOT%/}etc/isabelle/components" ]]; then
			...
			cat <<- EOF >> "${EROOT%/}etc/isabelle/components" || die "Failed to register CVC3 with Isabelle"
				contrib/${PN}-${PV}
			EOF
		fi
	fi
