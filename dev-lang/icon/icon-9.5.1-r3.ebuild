...
PATCHES=(
	...
	# https://bugs.gentoo.org/732506
	# Fix runtime error with large ints
	# Patches obtained from upstream, but they are not yet in a release
	"${FILESDIR}"/${PN}-9.5.1-minlong.patch
	"${FILESDIR}"/${PN}-9.5.1-cset-bitmask.patch
	"${FILESDIR}"/${PN}-9.5.1-signed-overflow.patch
	"${FILESDIR}"/${PN}-9.5.1-itos-strcpy.patch
)
