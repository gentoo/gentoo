...
RUST_MAX_VER=${PV}
if [[ ${PV} = *9999* ]]; then
	RUST_MIN_VER="1.84.0" # Update this as new versions are released
else
...
src_unpack() {
	if [[ ${PV} = *9999* ]]; then
		# we can skip a hefty fetch if we have an appropriate system LLVM installed.
		use system-llvm && EGIT_SUBMODULES+=( "-src/llvm-project" )
		git-r3_src_unpack
		mkdir ${S}/.cargo || die # The vendor script has a check for .cargo/config{,.toml}
		touch ${S}/.cargo/config.toml || die
		# since we're using git sources we need to run the Vendor script to ensure that all dependencies are present and up-to-date
		mkdir ${S}/vendor || die
		# TODO: This fetches a `beta` tarball, maybe we can patch that out and use our existing rust installs?
		# TODO: Investigate: this may do some light compilation - it doesn't actually build the sources but I noticed one or two
		# 'Compiled' messages in the output. This is probably fine, strictly speaking it's required to get the sources
		"${S}"/x.py vendor || die "Failed to vendor dependencies"
...
src_prepare() {
	if [[ ${PV} = *9999* ]]; then
		# We need to update / generate lockfile(s?)
		if ver_test ${RUST_SLOT} -lt "1.85.0"; then
			# We need to tell system-cargo that it's OK to use a later edition that is not stable
			# Kind of hacky but that's what you get when you're not using the official build proress!
			find "${S}" -type f -name Cargo.toml -not -path '*/vendor/*' \
				-exec sed -i '1i cargo-features = ["edition2024"]' {} \;
		fi
		# this should update all lockfiles in the workspace
		${CARGO} generate-lockfile --offline "${S}/Cargo.toml"
...
src_configure() {
	...
	local build_nightly="stable"
	local build_miri="false"
	local build_parallel_compiler="false"
	if [[ ${PV} = *9999* ]]; then
		build_nightly="nightly"
		build_miri="$(usex miri true false)"
		build_parallel_compiler="$(usex parallel-compiler true false)"
	fi
	cat <<- _EOF_ > "${S}"/config.toml
		...
		parallel-compiler = ${build_parallel_compiler}
		channel = "${build_nightly}"
		description = "gentoo"
		rpath = true
		verbose-tests = true
