...
src_compile() {
	...
	export RUSTC_VERSION=${MRUSTC_RUST_VER} # We're pretending that we're building against 1.74.0 to match patch version - it works
	...
	# build mrustc & minicargo then uses them to build the standard library
...
src_test() {
	...
	edo "${S}"/bin/mrustc -L "${S}"/output-${MRUSTC_RUST_VER}/ -g "${S}/../rustc-${RUST_VERSION}-src/tests/ui/hello_world/main.rs" \
		-o "${T}"/hello
