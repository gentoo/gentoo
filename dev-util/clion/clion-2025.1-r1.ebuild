...
src_prepare() {
	my_arch_x64_aarch64=$(usex amd64 x64 aarch64)
	my_arch_x64_arm64=$(usex amd64 x64 arm64)
	local other_arch_amd64_aarch64=$(usex amd64 aarch64 amd64)
	local other_arch_x64_arm64=$(usex amd64 arm64 x64)
	tc-export OBJCOPY
	default
	...
	local remove_me=(
		...
		plugins/clion-radler/DotFiles/linux-${other_arch_x64_arm64}
		plugins/clion-radler/dotTrace.dotMemory/DotFiles/linux-${other_arch_x64_arm64}
	...
	skip_nonx86_files=(
		...
		"plugins/clion-radler/DotFiles/linux-musl-${other_arch_x64_arm64}/jb_zip_unarchiver"
	...
	patchelf --set-rpath '$ORIGIN/../lib' "bin/clang/linux/${my_arch_x64_aarch64}/lib/libclazyPlugin.so" || die
}
...
src_install() {
	...
	insinto "${dir}"
	doins -r *
	fperms 755 "${dir}"/bin/{clion.sh,format.sh,fsnotifier,inspect.sh,jetbrains_client.sh,ltedit.sh,remote-dev-server.sh,restarter,clang/linux/${my_arch_x64_aarch64}/bin/{clangd,clang-tidy,clazy-standalone,llvm-symbolizer}}
	...
	fperms 755 "${dir}"/plugins/clion-radler/DotFiles/linux-${my_arch_x64_arm64}/Rider.Backend

	dosym -r "${EPREFIX}/usr/bin/ninja" "${dir}"/bin/ninja/linux/${my_arch_x64_aarch64}/ninja
	...
	dostrip -x "${skip_nonx86_files[@]/#//opt/clion/}"
}
