...
IUSE="clang debugger nsight profiler rdma sanitizer"
...
cuda-toolkit_check_reqs() {
	if use amd64; then
		local CHECKREQS_DISK_BUILD="6000M"
	elif use arm64; then
		local CHECKREQS_DISK_BUILD="5800M"
	fi
...
cuda_verify() {
	...
	compiler_versions="$(
		grep -oP "unsupported (GNU|clang) version.*(gcc versions later than|clang version must be less than) [0-9]*" \
			"${S}"/builds/cuda_crt/targets/*/include/crt/host_config.h
	)"
...
pkg_postinst() {
	...
	if use profiler; then
		...
		einfo "You'll need to run profiling tools "
