...
src_install() {
	...
	pushd builds/cuda_documentation
	...
	local builddirs=( builds/cuda_{cudart,cuobjdump,memcheck,nvcc,nvdisasm,nvml_dev,nvprune,nvrtc,nvtx} builds/lib{cublas,cufft,curand,cusolver,cusparse,npp,nvjpeg} $(usex profiler "builds/cuda_nvprof builds/cuda_cupti" "") $(usex vis-profiler "builds/cuda_nvvp" "")  $(usex debugger "builds/cuda_gdb" "") )
	for d in "${builddirs[@]}"; do
		ebegin "Installing ${d}"
		test -d "${d}" || die "Directory does not exist: ${d}"
	...
	if use nsight ; then
		...
		mv builds/nsight_compute builds/${ncu_dir}
		doins -r builds/${ncu_dir}

		exeinto ${cudadir}/${ncu_dir}
		for e in builds/${ncu_dir}/{ncu,ncu-ui,nv-nsight-cu,nv-nsight-cu-cli} ; do
			doexe ${e}
		done

		exeinto ${cudadir}/${ncu_dir}/host/linux-desktop-glibc_2_11_3-x64
		for e in builds/${ncu_dir}/host/linux-desktop-glibc_2_11_3-x64/{ncu-ui,ncu-ui.bin,CrashReporter} ; do
			doexe ${e}
		done
		for b in builds/integration/nsight-compute/{ncu,ncu-ui,nv-nsight-cu,nv-nsight-cu-cli} ; do
			dobin ${b}
		done
		eend
		...
		mv builds/nsight_systems builds/${nsys_dir}
		...
		for e in builds/${nsys_dir}/host-linux-x64/{nsight-sys,nsight-sys.bin,nsys-ui,CrashReporter,ImportNvtxt,QdstrmImporter,ResolveSymbols} ; do
			doexe ${e}
		done
		for b in builds/integration/nsight-systems/{nsight-sys,nsys,nsys-exporter,nsys-ui} ; do
			dobin ${b}
		done
		eend
		# TODO: unbundle qt5
