...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-610-fix-compiler-name-check.patch
	"${FILESDIR}"/${PN}-610-fix-build-for-clang16.patch
)

src_prepare() {
	# bug https://bugs.gentoo.org/858317
	filter-lto

	default
	sed -i -e 's^man/man1/biew.1^share/man/man1/biew.1^' makefile || die
	if [[ $CC == "clang" ]]; then
	  eapply "${FILESDIR}"/${PN}-610-fix-clang16-asm-definition.patch
	fi
}
