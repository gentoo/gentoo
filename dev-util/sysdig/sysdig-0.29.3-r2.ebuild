...
src_prepare() {
	...
	# force C++17 standard for libs & main
	sed -i -e 's:-std=c++0x:-std=c++17:' "${WORKDIR}"/libs-${LIBS_COMMIT}/cmake/modules/CompilerFlags.cmake || die
	sed -i -e 's:-ggdb::' -e 's:-std=c++0x:-std=c++17:' CMakeLists.txt || die
