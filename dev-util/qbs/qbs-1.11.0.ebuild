...
src_prepare() {
	...
	# qbsres target uses the qbs binary to bootstrap itself so, with rpath disabled, we need to preload libqbscore.so
	sed -i -e '\;qbsres.commands = ;a \    LD_PRELOAD=$$shell_quote($$shell_path($${QBS_LIBRARY_DIRNAME}/libqbscore.so)) \\' static-res.pro || die
}
