...
src_configure() {
	...
	# CPU settings
	### TODO set --arch and --tune too?
	### This seems complicated is there something already done by archtecture masking?
	### Bunch of other settings too. Ts this what flag-o-matic is used for?
	...
	myconf+=($(use_enable dvb))
	myconf+=($(use_enable asi))

	# Video Output Support
	myconf+=($(use_enable X x11))
	myconf+=($(use_enable X xnvctrl))
	myconf+=($(use_enable X xrandr))
	myconf+=($(use_enable X xv))

	# Hardware accellerators
	myconf+=($(use_enable vdpau))
	myconf+=($(use_enable crystalhd))
	myconf+=($(use_enable vaapi))
	myconf+=($(use_enable vaapi vaapi2))
	myconf+=($(use_enable opengl opengl-video))
	myconf+=($(use_enable opengl opengl-themepainter))
	myconf+=($(use_enable libass))

	myconf+=($(use_enable systemd systemd_notify))
	myconf+=($(use_enable systemd systemd_journal))
	myconf+=($(use_enable xml libxml2))
	...
	myconf+=($(use_enable lame libmp3lame))
...
src_install() {
	...
	if use autostart; then
		...
		# Check that ACCT_USER_HOME="/var/lib/mythtv" will work instead of "/home/mythtv"
		# And fix in acct-user/mythtv ebuild.
		# Add wirning about different home directory in installation message.
