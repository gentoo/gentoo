...
IUSE="accessibility aqua avif examples gamepad keyring +gstreamer +introspection pdf jpegxl +jumbo-build lcms seccomp spell systemd wayland X"
...
pkg_pretend() {
	if [[ ${MERGE_TYPE} != "binary" ]] ; then
		...
		die "You need at least GCC 12x or Clang >= 9 for C++23-specific compiler flags"
		fi
	fi
}
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DENABLE_SPEECH_SYNTHESIS=$(usex accessibility)
