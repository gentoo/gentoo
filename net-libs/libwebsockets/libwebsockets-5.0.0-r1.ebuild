...
PATCHES=(
	"${FILESDIR}"/${PN}-${PV}-dependency-fix.patch
	"${FILESDIR}"/${PN}-${PV}-prevent-account-creation.patch
)
...
src_configure() {
	...
	use mbedtls && mycmakeargs+=(
		-DLWS_MBEDTLS_INCLUDE_DIRS="/usr/include/mbedtls3"
		-DMBEDTLS_LIBRARY="mbedtls-3.so"
		-DMBEDX509_LIBRARY="mbedx509-3.so"
		-DMBEDCRYPTO_LIBRARY="mbedcrypto-3.so"
	)
