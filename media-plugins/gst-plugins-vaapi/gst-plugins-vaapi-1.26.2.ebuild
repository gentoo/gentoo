...
# gst-vaapi configure is based around GL platform mainly, unlike gst-plugins-bad
# that goes by GL API mainly; for less surprises, we design gst-vaapi ebuild in
# terms of GL API as main choice as well, meaning that USE opengl and/or gles2
# is required to enable opengl support at all and choices get chained from there.
#
# One or multiple video output are required: drm, x11, glx, egl and/or wayland;
# but GL API is our main trigger, thus USE=egl should be ineffective if neither
# gles2 or opengl is enabled; So "|| ( drm egl opengl wayland X )" would be wrong,
# because egl isn't built with USE="egl -opengl -gles2", ending up with no video
# outputs. As we ensure at least one working GL output with other REQUIRED_USE,
# we can put gles2/opengl in REQUIRED_USE instead of egl, solving the issue.
...
# - 'egl' is exposed as a USE flag mainly to get EGL support instead of or in addition
#    to GLX support with desktop GL while keeping it optional for pure GLX cases;
#    it's always required with USE=gles2, thus USE="gles2 opengl X" will require
#    and build desktop GL EGL platform support as well on top of GLX, which doesn't
#    add extra deps at that point.
# 'gles2' enables GLESv2 or GLESv3 based GL API support
# 'opengl' enables desktop OpenGL based GL API support
# 'wayland' enables non-GL Wayland support; wayland EGL support when combined with
#    'egl' (but only if also 'opengl||gles2')
