...
CEF_COMMIT="0d0eeb61160536e447c79335c1ee963f57eb6d60"
...
src_unpack() {
	unpack chromium-${PV}-linux.tar.xz
...
src_prepare() {
	...
	local keeplibs=(
		...
		# chromium needs the following only with USE=test
	...
	sed -i -e "s:@REV@:${CEF_BRANCH}:" tools/cef_version.py || die # used during installation
	popd
...
chromium_configure() {
	...
	# https://groups.google.com/a/chromium.org/g/blink-dev/c/wx0gZFCY3p4/m/fr2CNBQhAQAJ?pli=1
	# https://github.com/chromiumembedded/cef/issues/3803?timeline_page=1
	# https://bitbucket.org/chromiumembedded/cef/pull-requests/919/
	# https://bugs.gentoo.org/529604#c5
	myconf_gn+=( "blink_heap_inside_shared_library=true" )
	...
	# avoid passing --args (as in chromium ebuild) or GN_ARGUMENTS="--args=..."
...
chromium_compile() {
	...
	if use pax-kernel; then
		...
		for x in mksnapshot v8_context_snapshot_generator code_cache_generator; do
			if tc-is-cross-compiler; then
				eninja -C out/${OUT_CONFIG} "host/${x}"
				pax-mark m "out/${OUT_CONFIG}/host/${x}"
			else
				eninja -C out/${OUT_CONFIG} "${x}"
				pax-mark m "out/${OUT_CONFIG}/${x}"
			fi
		done
	fi
	...
	eninja -C out/${OUT_CONFIG} cefclient cefsimple $(use test && echo ceftests) chrome_sandbox

	pax-mark m out/${OUT_CONFIG}/chrome
...
chromium_profile() {
	...
	if ! "${EPYTHON}" ./chromium_profiler.py \
		--chrome-executable "${S}/out/${OUT_CONFIG}/chrome" \
		--chromedriver-executable "${S}/out/${OUT_CONFIG}/chromedriver.unstripped" \
...
src_test() {
	# doesn't work running in portage, here I inserted only the tests
	# to be filtered when running from regular user commandline
	local skip_tests=(
		'CertificateErrorTest.*:'
		CookieTest.AccessServerStandardGlobalAllow
		CookieTest.AccessServerStandardGlobalAllowNoFilter
		CookieTest.AccessServerStandardGlobalBlockRead
		CookieTest.AccessServerStandardGlobalBlockWrite
		CookieTest.AccessServerStandardGlobalBlockReadWrite
		CookieTest.AccessServerStandardInMemoryAllow
		CookieTest.AccessServerStandardInMemoryAllowNoFilter
		CookieTest.AccessServerStandardInMemoryBlockRead
		CookieTest.AccessServerStandardInMemoryBlockWrite
		CookieTest.AccessServerStandardInMemoryBlockReadWrite
		CookieTest.AccessServerStandardInMemoryBlockAllCookies
		CookieTest.AccessServerStandardGlobalAllowNoHandler
		CookieTest.AccessServerStandardInMemoryAllowNoHandler
		CookieTest.RestartGlobal
		CookieTest.RestartInMemory
		HSTSRedirectTest.Redirect
		'TestServer*.*Https*'
	)
	local test_filter="-$(IFS=:; printf '%s' "${skip_tests[*]}")"
	export LD_LIBRARY_PATH=./out/${OUT_CONFIG}/
	virtx ./out/${OUT_CONFIG}/ceftests \
		--test-launcher-jobs="$(get_makeopts_jobs)" \
		--no-sandbox --gtest_filter=$test_filter || die "test failed"
}

src_install() {
	local DISTRIB_DIR="${T}"/distrib
	local CEF_HOME="/usr/$(get_libdir)/${PN}"

	exeinto "${CEF_HOME}"
	doexe out/${OUT_CONFIG}/cefclient
	doexe out/${OUT_CONFIG}/cefsimple
	for bin in cefclient cefsimple
	do
		cat <<-EOF > "${TEMP}"/$bin
			LD_LIBRARY_PATH=${CEF_HOME} ${CEF_HOME}/$bin
			EOF

		dobin "${TEMP}"/$bin
	done

	pushd "${DISTRIB_DIR}"/Resources/locales > /dev/null || die
	chromium_remove_language_paks
	popd > /dev/null || die

	# cmake files have hardcoded paths to ${CEF_HOME} subdirs
	insinto "${CEF_HOME}"
	doins -r "${DISTRIB_DIR}"/Release/*
	doins -r "${DISTRIB_DIR}"/Resources/*
	doins -r "${DISTRIB_DIR}"/cmake
	doins -r "${DISTRIB_DIR}"/tests
	...
	dodoc "${DISTRIB_DIR}"/README*

	if use doc ; then
		docinto html
		dodoc -r "${DISTRIB_DIR}_docs"/*
	fi
}
