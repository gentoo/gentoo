...
SRC_URI="${MOZ_HTTP_URI}/source/${MY_MOZ_P}.source.tar.xz -> ${P}.source.tar.xz
	...
	https://github.com/BioMike/gentoo-${PN}-patches/archive/refs/tags/${PVR}.tar.gz -> ${PATCH}.tar.gz"
...
src_prepare() {
	# Apply our patches
	eapply "${WORKDIR}"/gentoo-${PN}-patches-${PVR}/${PN}
	...
	use system-libvpx && eapply -p2 "${WORKDIR}"/gentoo-${PN}-patches-${PVR}/USE_flag/1009_seamonkey-2.53.3-system_libvpx-1.8.patch
