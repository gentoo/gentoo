...
PATCHES=(
	"${FILESDIR}"/"${PN}"-fix-docdir-path.patch
	"${FILESDIR}"/"${PN}"-enable-failed-mlock-warning.patch
)
...
src_prepare() {
	...
	if ! use systemd; then
		...
		eapply -p1 "${FILESDIR}"/"${PN}"-non-systemd-integration.patch
	fi
}
...
#		-Dlibjack-path="" # Where to install libjack.so et al (if an absolute path is used, remember to prefix it with ${EROOT} or similar!); setting this will also break pw-jack's multilib support (but presumably that's okay as the intended use would be to replace system's libraries making the loader irrelevant)
		...
		-Dudev=enabled
#		-Dudevrulesdir="" # Directory for udev rules (defaults to /lib/udev/rules.d)
#		-Dsystemd-user-unit-dir="" # Directory for user systemd units (defaults to pkg-config systemd --variable=systemduserunitpath ; prefix awareness expected but not verified)
...
pkg_postinst() {
	...
	if use systemd; then
		...
		elog "autospawn = no"
		elog "Please note that ; must _NOT_ be at the beginning of the line!"
		elog
