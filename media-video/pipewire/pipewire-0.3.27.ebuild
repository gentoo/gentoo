...
src_compile() {
	...
	cat > ${limitsdfile} <<- EOF || die
		...
		# Required to memlock buffers for pipewire-pulse & JACK emulation
		@pipewire-desktop	-	memlock	256

		# Should match Red Hat defaults for their realtime group
		# If 40-realtime-base.conf exists, it will override these with higher values
		# Subject to be changed (lowered) as a more reasonable upper bound is determined
		@realtime		-	memlock	4194304
		@realtime		-	rtprio	70
...
pkg_postinst() {
	...
	elog "JACK emulation is incomplete and not all programs will work. PipeWire's JACK"
	elog "libraries have been installed outside LD search paths. To use them, put"
	elog "pw-jack <application> in front of every JACK application. When using pw-jack,"
	elog "do not run jackd/jackdbus. However, a virtual/jack provider is still needed to"
	elog "compile the JACK applications themselves. A few JACK applications may require"
	elog "the user to be in the realtime group to function correctly."
	elog
	...
	elog "For best experience add all PipeWire users to the pipewire-desktop group."
