...
FFMPEG_IUSE_MAP=(
	...
	openmpt:libopenmpt
	+openssl:@v3ifgpl # unused if USE=gnutls, LGPL2.1+ if USE=-gpl
	opus:libopus
...
LICENSE="
	gpl? (
		...
		gmp? ( GPL-3+ ) openssl? ( !gnutls? ( GPL-3+ ) )
		fdk? ( all-rights-reserved )
		npp? ( all-rights-reserved )
	)
	!gpl? (
		LGPL-2.1+
...
COMMON_DEPEND="
	...
	${FFMPEG_SOC_PATCH:+"
		soc? (
			...
			X? ( media-libs/libepoxy[${MULTILIB_USEDEP}] )
		)
	"}
"
RDEPEND="
...
multilib_src_configure() {
	...
	conf+=(
		...
		--disable-librist # needs handling but no amd64 keyword? (bug #822012)
	...
	in_iuse soc && use soc &&
		conf+=( --enable-libudev --enable-sand --enable-v4l2-request )
