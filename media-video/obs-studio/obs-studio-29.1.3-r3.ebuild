...
inherit cmake desktop lua-single optfeature python-single-r1 xdg
...
SLOT="0"
IUSE="
	+alsa amf browser decklink fdk jack lua mpegts nvenc pipewire pulseaudio
	python qt6 speex +ssl truetype v4l vlc wayland websocket
"

# need to use qt6 to prevent obs from fail to open
REQUIRED_USE="
...
src_prepare() {
	...
	if use amf; then
		eapply "${FILESDIR}/${PN}-amf-${PV}.patch"
	fi
...
src_install() {
	...
	#obs desktop file to use AMDGPU PRO Vulkan icd loader (needed for amf)
	#overrites regular dekstop file since it dosent affect other encoder
	use amf && newmenu "${FILESDIR}/com.obsproject.Studio.amf.desktop" com.obsproject.Studio.desktop
}
