...
inherit cargo cmake flag-o-matic lua-single toolchain-funcs xdg-utils
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-1.0.2-glext-prototypes.patch"
)
...
src_install() {
	...
	# FIXME: these files should not be installed
	rm -f "${ED}"/usr/bin/hwengine_future_install_target.cmake || die
	rm -f "${ED}"/usr/bin/qt.conf || die
	find "${ED}/usr/$(get_libdir)"/ -mindepth 1 \
		! -name "libavwrapper.*" ! -name "libphyslayer.*" ! -name "libhwengine_future.*" \
		-delete || die
}
