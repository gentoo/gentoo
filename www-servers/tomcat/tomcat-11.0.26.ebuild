...
COMMON_DEP="
	...
	>=dev-java/bnd-annotation-${BND_MIN}:0
	dev-java/eclipse-ecj:${ECJ_SLOT}
	dev-java/jax-rpc-api:0
	>=dev-java/jakartaee-migration-1.0.12:0
	>=dev-java/wsdl4j-1.6.3-r1:0"
...
DEPEND="
	${COMMON_DEP}
	app-admin/pwgen
	>=dev-java/bnd-${BND_MIN}:0
	>=dev-java/bnd-ant-${BND_MIN}:0
	>=dev-java/bnd-util-${BND_MIN}:0
	>=dev-java/bndlib-${BND_MIN}:0
	>=dev-java/libg-${BND_MIN}:0
	dev-java/osgi-cmpn:8
	dev-java/osgi-core:0
	dev-java/slf4j-api:0
	|| ( virtual/jdk:21 virtual/jdk:17 )
	test? (
		>=dev-java/ant-1.10.15-r1:0[junit]
		>=dev-java/easymock-5.7.0:0
	)"
