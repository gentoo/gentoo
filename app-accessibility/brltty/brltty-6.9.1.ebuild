...
src_configure() {
	...
	use speech && myconf+=(
		# espeak-ng installs a wrapper espeak, auto-detected by brltty
		# fix autodetection w/ a custom ROOT and then disable espeak
		--with-espeak="${ESYSROOT}/usr"
		--with-espeak_ng="${ESYSROOT}/usr"
		--with-flite="${ESYSROOT}/usr"
...
src_install() {
	...
	if use python; then
		python_install() {
			emake -C "${BUILD_DIR}"/Bindings/Python INSTALL_ROOT="${D}" install
