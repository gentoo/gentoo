...
LICENSE="LGPL-2.1+"
SLOT="2"
IUSE="X dbus_broker gtk-doc +introspection systemd"
REQUIRED_USE="dbus_broker? ( systemd ) gtk-doc? ( X )"
KEYWORDS="~alpha ~amd64 ~arm ~arm64 ~hppa ~ia64 ~loong ~mips ~ppc ~ppc64 ~riscv ~s390 ~sparc ~x86 ~amd64-linux ~x86-linux ~x64-macos"

RDEPEND="
	dbus_broker? ( sys-apps/dbus-broker[${MULTILIB_USEDEP}] )
	!dbus_broker? ( >=sys-apps/dbus-1.5[${MULTILIB_USEDEP}] )
...
multilib_src_configure() {
	local emesonargs=(
		-Ddefault_bus=$(usex dbus_broker dbus-broker dbus-daemon)
