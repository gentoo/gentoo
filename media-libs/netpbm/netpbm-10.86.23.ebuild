...
IUSE="doc jbig jpeg png postscript rle cpu_flags_x86_sse2 static-libs svga tiff X xml"
...
# returns -l${1} if any of ${2}... is used
netpbm_config_any() {
	local lib=${1}
	while [ -n "${2}" ]; do
		if use ${2}; then
...
src_configure() {
	...
	ZLIB = $(netpbm_config_any z png postscript xml)
