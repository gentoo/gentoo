...
COMMON_DEPEND="dev-cpp/libxmlpp:2.6[${MULTILIB_USEDEP}]
	dev-libs/dbus-c++
...
src_prepare() {
	...
	# Python3 fixes
	sed -i -e 's/\t/        /g' "${S}"/support/mixer-qt4/ffado/mixer/phase88control.py || die
	sed -i -e 's/\t/        /g' "${S}"/support/mixer-qt4/ffado/mixer/audiofire.py || die
	sed -i -e 's/\bprint \(.*\)$/print(\1)/g' -e '/import commands/d' "${S}"/support/tools/ffado-diag-static || die

	# Always use Qt5
	sed -i -e 's/try:/if False:/' -e 's/except.*/else:/' "${S}"/support/mixer-qt4/ffado/import_pyqt.py || die
