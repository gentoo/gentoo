...
MY_P=tiff-${PV}
DESCRIPTION="ABI compatibility package providing libtiff.so.3"
...
src_prepare() {
	epatch \
		"${FILESDIR}"/${MY_P}-CVE-2012-{4447,4564,5581}.patch \
		"${FILESDIR}"/${MY_P}-tiffinfo-exif.patch \
		"${FILESDIR}"/${MY_P}-printdir-width.patch
