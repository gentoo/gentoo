...
PATCHES=(
	...
	"${FILESDIR}/${PN}-4.14.0-use-system-flatbuffers.patch"
	"${FILESDIR}/${PN}-4.14.0-openjpeg-version.patch"
...
multilib_src_test() {
	...
	if multilib_native_use cuda; then
		...
		if use opengl; then
			...
			# tinywl's headless backend leaves namedWindow(WINDOW_OPENGL)
			# without a GL context, so every cv::ogl test dies on bufId_ != 0
			use wayland && OPENCV_SKIP_TESTS_cudaarithm=( 'OpenGL/*' )
		fi
	fi
