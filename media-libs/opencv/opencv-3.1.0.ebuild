...
inherit toolchain-funcs cuda cmake-utils python-r1 java-pkg-opt-2 java-ant-2
...
SRC_URI="
	https://github.com/${PN}/${PN}/archive/${PV}.zip -> ${P}.zip
	contrib? (
		https://github.com/${PN}/${PN}_contrib/archive/{PV}.zip -> ${P}_contrib.zip )"
...
src_prepare() {
	...
	if use contrib; then
		...
		epatch "${FILESDIR}"/${P}-sfm-gflags.patch
	fi
}
...
src_configure() {
	...
	if use qt5 && use vtk ; then
		GLOBALCMAKEARGS+=( -DFORCE_VTK=ON )
	fi
