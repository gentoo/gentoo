...
# 2. a downloaded, unverified bundle of active infra developers from qa-reports.gentoo.org (https://qa-reports.gentoo.org/output/infra-devs.gpg);
...
#
# Usage: python keyring-mangler.py <gentoo-auth.asc> <infra-devs.gpg> <output for armored keys.asc>
...
for key in gpg.list_keys(sigs=True):
    ...
    if key["fingerprint"] in SKIP_KEYS:
        # We don't want the other L2 keys (e.g. developers) in here as we're filtering
        # for infra members.
        continue
