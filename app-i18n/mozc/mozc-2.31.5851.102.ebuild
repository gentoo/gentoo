...
SRC_URI="
	...
	amd64? ( https://releases.bazel.build/${BAZEL_VER}/release/bazel-${BAZEL_VER}-linux-x86_64 )
	arm64? ( https://releases.bazel.build/${BAZEL_VER}/release/bazel-${BAZEL_VER}-linux-arm64 )
...
SLOT="0"
KEYWORDS="~amd64 ~arm64" # no x86 binary for bazel
...
src_unpack() {
	...
	if use fcitx5; then
		...
		ln -sf "${WORKDIR}"/${PN}-${MOZC_FCITX_HASH} "${WORKDIR}"/${P} || die
	else
		unpack ${P}.tar.gz
	fi
}
