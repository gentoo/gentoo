...
pkg_postinst() {
	...
	if ! use systemd; then
		elog "The init script (/etc/init.d/kresd) set capabilities by default in order to"
		elog "bind daemon to well-known ports. Check that init system support it if needed."
	fi
}
