...
HOMEPAGE="https://nlnetlabs.nl/projects/unbound/about/ https://github.com/NLnetLabs/unbound"
SRC_URI="
...
IUSE="debug dnscrypt dnstap +ecdsa ecs gost +http2 python quic redis selinux static-libs systemd test +tfo"
...
# is fixed.
#
# The QUIC support requires net-libs/ngtcp2 with the OpenSSL backend.
DEPEND="
	acct-group/unbound
	acct-user/unbound
	...
	python? ( ${PYTHON_DEPS} )
	quic? (
		net-libs/ngtcp2:=[${MULTILIB_USEDEP},ssl(+),openssl(+)]
		>=dev-libs/openssl-3.2:=[${MULTILIB_USEDEP}]
	)
...
multilib_src_configure() {
	local myeconfargs=(
		...
		$(multilib_native_use_with quic libngtcp2)
