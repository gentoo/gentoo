...
IUSE="debug dnscrypt dnstap +ecdsa ecs gost +http2 lib-only python redis selinux static-libs systemd test +tfo threads"
...
multilib_src_configure() {
	econf \
		$(use_with lib-only libunbound-only) \
...
multilib_src_install_all() {
	...
	if ! use lib-only ; then # START LIB-ONLY
