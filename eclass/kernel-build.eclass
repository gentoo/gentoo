...
kernel-build_src_configure() {
	...
	if [[ ${PV} != *9999 ]]; then
		...
		if [[ ${KV_FULL} != ${expected_ver}* ]]; then
			...
			eerror "Got:      ${KV_FULL}"
			eerror "Expected: ${expected_ver}*"
			eerror "Kernel kernel version must match PV."
			eerror "Please ensure you are applying the correct"
			eerror "patches and that PV matches the prepared"
			eerror "kernel's version."
