...
dkms_gentoofy_conf() {
	...
	for file in "${input[@]}"; do
		...
		if ! grep -q "^MAKE\[0\]=.*${edkmsargs[*]}" "${file}" &&
			! grep -q "^MAKE=.*${edkmsargs[*]}" "${file}"
		then
			sed -i "${file}" \
				...
				-e "/^MAKE/ s:'make'$:'make' ${edkmsargs[*]}:" \
				-e "/^MAKE/ s:'make'\":'make' ${edkmsargs[*]}\":" ||
					die "${FUNCNAME}: failed to sed ${file}"
		fi
	done
}
...
# edkms_args array. This array contains all variables from the two
...
dkms_autoconf() {
	...
	local edkms_args index mod name package
	for mod in "${modlist[@]}"; do
		...
		if [[ -f dkms.conf ]]; then
			if grep -qE "^MAKE(\[0\]|)=" dkms.conf; then
				...
				dkms_sanitize_makeargs
				echo "MAKE[0]=\"${make_command} ${edkmsargs[*]} ${mod[3]}\"" \
					>> dkms.conf || die
			fi
			...
			cat <<-EOF > dkms.conf || die
				...
				PACKAGE_VERSION=${PV}
				MAKE[0]="${make_command} ${edkmsargs[*]} ${mod[3]}"
			EOF
			if [[ -n ${autoinstall} ]]; then
				echo "AUTOINSTALL=yes" >> dkms.conf
			else
				echo "AUTOINSTALL=no" >> dkms.conf
			fi
			index=0
		fi
...
dkms_dopackage() {
	...
	local package="$(
		...
		dest=${ED}/usr/src/${PACKAGE_NAME}-${PACKAGE_VERSION}
		mkdir -p "${dest}" || die
		cp -a "${package_root}"/* "${dest}" || die
...
dkms_src_install() {
	...
	use dkms || linux-mod-r1_src_install
	[[ -z ${MODULES_OPTIONAL_IUSE} ]] ||
		use "${MODULES_OPTIONAL_IUSE#+}" || return 0
...
dkms_pkg_postinst() {
	...
	use dkms || linux-mod-r1_pkg_postinst
	[[ -z ${MODULES_OPTIONAL_IUSE} ]] ||
		use "${MODULES_OPTIONAL_IUSE#+}" || return 0
