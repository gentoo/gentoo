...
make_desktop_entry() {
	...
	if [[ ${eapi9} ]]; then
		if [[ -z ${appid} ]]; then
			[[ ${cmd} =~ .+[[:space:]].+ ]] && die "make_desktop_entry: --appid must be provided when <command> contains a space"
			appid="${cmd##*/}"
		fi
		if [[ ${_DESKTOP_IDS[*]} =~ (^|[[:space:]])"${appid}"($|[[:space:]]) ]]; then
			die "make_desktop_entry: appid \"${appid}\" already used in a previous call, choose a different one"
		else
			_DESKTOP_IDS+=( "${appid}" )
		fi
