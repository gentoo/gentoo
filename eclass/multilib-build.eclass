	...
	if [[ ${MULTILIB_WRAPPED_HEADERS[@]} ]]; then
		...
		if [[ ${MULTILIB_ABI_FLAG} ]]; then
			for f in "${MULTILIB_WRAPPED_HEADERS[@]}"; do
				...
				MULTILIB_WRAPPED_HEADER_DIR="${MULTILIB_WRAPPED_HEADER_DIR:=usr/include}"

				# drop leading slash if it's there
				MULTILIB_WRAPPED_HEADER_DIR="${MULTILIB_WRAPPED_HEADER_DIR#/}"

				# drop trailing slash if it's there
				MULTILIB_WRAPPED_HEADER_DIR="${MULTILIB_WRAPPED_HEADER_DIR%/}"

				if [[ ${f} != ${MULTILIB_WRAPPED_HEADER_DIR}/* ]]; then
					die "MULTILIB_WRAPPED_HEADER_DIR=/${MULTILIB_WRAPPED_HEADER_DIR} not in header path"
				fi
				# get path offset from MULTILIB_WRAPPED_HEADER_DIR
				f=${f#${MULTILIB_WRAPPED_HEADER_DIR}}

				local dir=${f%/*}

				# Some ABIs may have install less files than others.
				if [[ -f ${root}/${MULTILIB_WRAPPED_HEADER_DIR}${f} ]]; then
					local wrapper=${ED}/tmp/multilib-include/${MULTILIB_WRAPPED_HEADER_DIR}${f}

					if [[ ! -f ${ED}/tmp/multilib-include/${MULTILIB_WRAPPED_HEADER_DIR}${f} ]]; then
						dodir "/tmp/multilib-include/${MULTILIB_WRAPPED_HEADER_DIR}${dir}"
						# a generic template
...
_EOF_
					...
					dodir "/tmp/multilib-include/${MULTILIB_WRAPPED_HEADER_DIR}/${CHOST}${dir}"
					mv "${root}/${MULTILIB_WRAPPED_HEADER_DIR}${f}" \
						"${ED}/tmp/multilib-include/${MULTILIB_WRAPPED_HEADER_DIR}/${CHOST}${dir}/" || die
