...
# @ECLASS: tree-sitter-grammar-r1.eclass
# @MAINTAINER:
...
tree-sitter-grammar-r1_src_prepare() {
...
tree-sitter-grammar-r1_src_configure() {
	debug-print-function ${FUNCNAME} "${@}"
...
tree-sitter-grammar-r1_src_compile() {
	debug-print-function ${FUNCNAME} "${@}"
...
tree-sitter-grammar-r1_src_test() {
	debug-print-function ${FUNCNAME} "${@}"

	tree-sitter test || die "Test suite failed"
}

tree-sitter-grammar-r1_src_install() {
	debug-print-function ${FUNCNAME} "${@}"
	...
	# Install a symlink to /usr/$(get_libdir)/tree-sitter/parser so that
	# NeoVim can find the parser library. The grammar doesn't start with
	# a "libtree-sitter-" prefix.
	dosym ../../lib${PN}$(get_libname) \
		  "/usr/$(get_libdir)/tree-sitter/parser/${PN##tree-sitter-}$(get_libname)"

	# If queries are present, install them to
	# /usr/share/tree-sitter/LANG/queries.
	if [[ -d "${S}/queries" ]]; then
		keepdir /usr/share/tree-sitter/${PN##tree-sitter-}
		insinto /usr/share/tree-sitter/${PN##tree-sitter-}
		doins -r "${S}/queries"
	fi
