...
#
# The 'edo' command should be used only where needed to give a more
# verbose log, e.g. for invoking non-standard ./configure scripts,
# or building objects/binaries directly within ebuilds via compiler
# invocations.  It is NOT to be used in place of generic 'command || die'
# where verbosity is unnecessary.
#
...
# @USAGE: <command> [<args>...]
# @DESCRIPTION:
# Executes a short 'command' with any given arguments and exits on
# failure unless called under 'nonfatal'.
edo() {
...
# exits on failure unless called under 'nonfatal'.  This function
# redirects stdout and stderr to a log file.  The content of the log
# file is shown if the command returns with a non-zero exit status.
#
# If -m <message> is provided, then invokes ebegin with <message>,
# otherwise a default message is used.  If -l <log-name> is provided,
# then <log-name> is used to construct the name of the log file where
# stdout and stderr of the command is redirected to.
edob() {
	local message
	local log_name
