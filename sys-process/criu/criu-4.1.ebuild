...
PATCHES=(
	...
	"${FILESDIR}/criu-4.1-glibc-rseq.patch"
...
src_prepare() {
	...
	if ! use doc; then
		sed -i 's_\(install: \)install-man _\1_g' Makefile.install || die
	fi
}
...
src_configure() {
	...
	# The build system is quite sensitive to weird CFLAGS. It may
	# show up with strange build errors (e.g. redefined enums) because
	# of confused 'configure' (Makefile) tests.
...
src_compile() {
	local -a targets=(
		all
		$(usev video_cards_amdgpu 'amdgpu_plugin')
		$(usev doc 'docs')
	)
	criu_emake "${targets[@]}"
