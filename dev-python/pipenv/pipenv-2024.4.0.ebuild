...
PATCHES=(
	"${FILESDIR}/pipenv-2024.4.0-inject-system-packages.patch"
	"${FILESDIR}/pipenv-2024.4.0-append-always-install-to-pip-extra-args.patch"
)
...
src_prepare() {
	...
	# remove plugin config from pytest in pyproject.toml
	sed -i '/ddopts = "-ra --no-cov"/d' pyproject.toml || die
	sed -i '/plugins = "xdist"/d' pyproject.toml || die
