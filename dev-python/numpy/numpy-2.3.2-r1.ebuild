...
python_configure_all() {
	...
	case ${ARCH} in
		...
		arm64)
			...
			if [[ ${cpu_baseline[-1]} == NEON_VFPV4 ]] || use arm64; then
				cpu_baseline+=( $(usev cpu_flags_arm_asimd ASIMD) )
			fi

			# these two imply ASIMD
			if [[ ${cpu_baseline[-1]} == ASIMD ]]; then
			...
			if [[ ${cpu_baseline[-1]} == ASIMDHP ]]; then
		...
		amd64|x86)
			...
			if [[ ${cpu_baseline[-1]} == AVX512CD ]]; then
