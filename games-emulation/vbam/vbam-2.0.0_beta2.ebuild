...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="https://github.com/visualboyadvance-m/visualboyadvance-m/archive/VBA-M_Beta_2.tar.gz"
	KEYWORDS="~amd64 ~x86"
fi
...
S="${WORKDIR}/visualboyadvance-m-VBA-M_Beta_2"
...
src_prepare() {
	...
	sed -i "s:\(DESTINATION\) bin:\1 ${EPREFIX}/bin:" \
		CMakeLists.txt src/{wx,gtk}/CMakeLists.txt || die
	eapply "${FILESDIR}"/vbam-2.0.0_pre1507-man.patch
	eapply "${FILESDIR}"/${P}-wx-cmakelists.patch

	eapply_user
}

src_configure() {
	append-cxxflags $(test-flags-CXX -std=gnu++11 -fpermissive)
	local mycmakeargs=(
