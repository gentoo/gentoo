...
IUSE="all-modules aqua boost cuda doc examples ffmpeg gdal imaging java json mpi
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-8.2.0-freetype-2.10.3-provide-FT_CALLBACK_DEF.patch # bug #751088
	"${FILESDIR}"/${PN}-9.0.1-build-type.patch
)
...
src_prepare() {
	...
	# unremoved 3rd party: verdict vpic vtkm xdmf3
	#
	local VTK_DROP=( diy2 doubleconversion eigen exodusII expat freetype
	                 gl2ps glew h5part hdf5 jpeg jsoncpp kissfft libharu
	                 libproj libxml2 loguru lz4 lzma mpi4py netcdf ogg
			 pegtl png pugixml sqlite theora utf8 xdmf2 tiff zfp zlib )
	local x
	for x in ${VTK_DROP[@]}; do
		elog "Dropping bundled ${x}"
		rm -rf ThirdParty/${x}/vtk${x} || die "failed to drop ${x}"
	done

	if use doc; then
		elog "Removing .md5 files from documents."
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DVTK_MODULE_USE_EXTERNAL_utf8cpp=ON
