...
# requires colpack build with cmake
RDEPEND="
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-2.7.2-boost-1.70.patch"
)
...
src_configure() {
	...
	local myeconfargs=(
		...
		--enable-atrig-erf
		--with-boost-system=header-only
		$(use_enable mpi ampi)
		$(use_enable sparse)
		$(use_with boost)
		$(use_with sparse colpack "${ESYSROOT}"/usr)
		--prefix="${EPREFIX}/usr"
	)
