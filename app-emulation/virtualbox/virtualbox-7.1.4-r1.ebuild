...
PATCHES=(
	"${FILESDIR}/virtualbox-7.1.4-java.patch"
	# Downloaded patchset
...
src_prepare() {
	...
	# add correct java path and prepare unbundling
	if use java; then
		sed "s@/usr/lib/jvm/java-6-sun@$(java-config -O)@" \
			-i "${S}"/Config.kmk || die

		# all jars in that directory should be unbundled
		java-pkg_clean ! -path "./src/VBox/Main/webservice/*"
...
src_compile() {
	...
	# add -source -target values from java eclasses
	if use java; then
		myemakeargs+=(
			VBOX_JAVA_VERSION=110000
			VBOX_JAVAC_OPTS="-encoding UTF-8 -source $(java-pkg_get-source) -target $(java-pkg_get-target) -Xlint:unchecked"
		)
	fi

	if use vboxwebsrv; then
		GENTOO_JWS_API_PATH="$(java-pkg_getjars jakarta-jws-api-javax)"
		GENTOO_JAXB_API_PATH="$(java-pkg_getjars jaxb-api-2)"
		GENTOO_JAXWS_API_PATH="$(java-pkg_getjars jaxws-api)"
	fi
