...
src_unpack() {
	if [[ ${PV} == *9999 ]]; then
		local EGIT_CHECKOUT_DIR=${WORKDIR}/${P}
		git-r3_src_unpack

		local EGIT_COMMIT=$("${EGIT_CHECKOUT_DIR}"/patches/patchinstall.sh --upstream-commit || die)
		local EGIT_REPO_URI=${WINE_EGIT_REPO_URI}
		EGIT_CHECKOUT_DIR=${S}
