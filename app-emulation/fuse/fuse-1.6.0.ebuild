...
IUSE="alsa ao backend-fbcon backend-sdl backend-svga backend-X gpm joystick memlimit png pulseaudio xml"

# Only one UI back-end can be enabled at a time
# At most one audio driver can be enabled at a time
...
src_configure() {
	...
	if use alsa; then
		myconf+=("--with-audio-driver=alsa")
	elif use ao; then
		myconf+=("--with-audio-driver=libao")
	elif use pulseaudio; then
		ewarn "PulseAudio support is experimental"
		myconf+=("--with-audio-driver=pulseaudio")
	else
		myconf+=("--with-audio-driver=null")
	fi
