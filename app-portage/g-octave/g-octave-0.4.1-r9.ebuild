...
pkg_config() {
	...
	rm -r "${db}"/{patches,octave-forge,info.json,manifest.json,timestamp}
	mv "${db}/${DB_DIR}"/* "${db}" || die 'mv failed.'
	rm -r "${db}/${DB_DIR}" || die 'rm db dir failed.'
	sed -i -e 's^sys-libs/ncurses^sys-libs/ncurses-compat:5^' "${db}"/info.json \
		|| die 'failed to patch info.json'
}
