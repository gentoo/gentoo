		...
		if [[ -d "${dir}" ]] ; then
			...
			if [[ -d "${sdir}/messages" ]] ; then
				...
				find "${sdir}"/messages/kdenetwork -type f ! \( -name CMakeLists.txt \
					...
					-delete
				if [[ -z $(find "${sdir}"/messages/kdenetwork/ -name "*po" -print -quit) ]]; then
					ewarn "${lng} does not contain translations for ${PN} and should be removed from L10N."
					rm "${sdir}"/messages/kdenetwork/CMakeLists.txt || die
				fi
			fi
		fi
	done
