...
SRC_URI="https://dl.xpdfreader.com/${P}.tar.gz
	i18n? (
		https://dl.xpdfreader.com/xpdf-arabic.tar.gz -> xpdf-arabic-20260323.tar.gz
		...
		https://dl.xpdfreader.com/xpdf-thai.tar.gz -> xpdf-thai-20250813.tar.gz
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-4.06-shared-libs.patch
	"${FILESDIR}"/${PN}-4.06-font-paths.patch
)
...
src_compile() {
	...
	if use icons; then
		...
		for i in ${SIZES}; do
			mkdir ${i} || die
			rsvg-convert xpdf-icon.svg -w ${i} -h ${i} -o ${i}/xpdf.png || die
		done
		popd || die
	fi
}
