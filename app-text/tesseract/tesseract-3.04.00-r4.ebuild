...
inherit eutils autotools autotools-utils java-pkg-opt-2
...
DEPEND=">=media-libs/leptonica-1.71[zlib,tiff?,jpeg?,png?,webp?]
		...
		scrollview? ( >=dev-java/piccolo2d-3.0 )
		training? (
		  dev-libs/icu
		  x11-libs/pango
		  x11-libs/cairo
...
src_compile() {
	pushd "${BUILD_DIR}" || die
	default
	if use scrollview; then
		emake ScrollView.jar
	fi
...
src_install() {
	...
	if use scrollview; then
		...
		#Neat trick to retrieve scrollview_path from the makefile. Thanks to http://blog.melski.net/2010/11/30/makefile-hacks-print-the-value-of-any-variable/
		local scrollview_path=$(make -f <(echo -e "ebuild_print-%:\n\t@echo '\$(\$*)'") -f java/Makefile ebuild_print-scrollview_path)
		insinto ${scrollview_path}/
		doins "java/ScrollView.jar"
		popd || die
	fi
