...
if [[ "${PV}" == "9999" ]]; then
	...
	LIBUNWIND_COMMIT="5afe6d8"  # patched
	LIBXML_COMMIT="18890f4"  # 2.9.8
	LZ4_COMMIT="7a4e3b1"  # almost 1.9.1
	MARIADB_CONNECTOR_C_COMMIT="c6503d3"  # patched pre-3.1.0
	ORC_COMMIT="5981208"  # somewhat before 1.5.1
	POCO_COMMIT="6216cc0"  # highly patched
	PROTOBUF_COMMIT="1273537"  # stripped protobuf-cpp@3.6.1
	...
	SIMDJSON_COMMIT="e9be643"  # unstable, newer than v0.2.1
	...
	SRC_URI="https://github.com/${MY_PN}/${MY_PN}/archive/v${PV}-${TYPE}.tar.gz -> ${P}.tar.gz
		...
		json? (
			!cpu_flags_x86_avx2? ( https://github.com/Tencent/rapidjson/archive/${RAPIDJSON_COMMIT}.tar.gz -> rapidjson-${RAPIDJSON_COMMIT}.tar.gz )
			cpu_flags_x86_avx2? ( https://github.com/lemire/simdjson/archive/${SIMDJSON_COMMIT}.tar.gz -> simdjson-${SIMDJSON_COMMIT}.tar.gz )
	)
...
CLICKHOUSE_DEPS=(
	...
	"capnp			capnp		>=dev-libs/capnproto-0.7.0[static-libs] \
	...
	"libcxx			libcxx		>=sys-libs/libcxx-9.0.0[libcxxabi,static-libs] \
		USE_INTERNAL_LIBCXX_LIBRARY=no		USE_LIBCXX		USE_LIBCXX"
...
src_unpack() {
	...
	if use json; then
		unpack_or_remove+=(
			"rapidjson	rapidjson-${RAPIDJSON_COMMIT}.tar.gz	!cpu_flags_x86_avx2"
			"simdjson	simdjson-${SIMDJSON_COMMIT}.tar.gz	cpu_flags_x86_avx2"
		)
	else
		# just remove them
		unpack_or_remove+=(
			"rapidjson	rapidjson-${RAPIDJSON_COMMIT}.tar.gz	json"
...
src_prepare() {
	...
	mysed '(#if !UNBUNDLED)' '#if 0  // \1 // ebuild: always use proper allocator' dbms/src/Common/new_delete.cpp

	mysed '(install \(EXPORT global DESTINATION cmake\))' 'message("Disabled \1")' CMakeLists.txt

	mysed 'if \(USE_STATIC_LIBRARIES\)' 'if (MAKE_STATIC_LIBRARIES)' cmake/find/zlib.cmake
	...
	mysed '(set_target_properties\(zlib zlibstatic PROPERTIES OUTPUT_NAME) z\$\{SUFFIX}\)' \
		'\1 z-ng) # ebuild: force library name' contrib/zlib-ng/CMakeLists.txt

	mysed '(set_source_files_properties.+|if.+|endif.+)' '# \1 # ebuild ' contrib/fastops-cmake/CMakeLists.txt
	...
	if use '!static' && { use jemalloc || use tcmalloc; }; then
		...
		use odbc && echo "target_link_libraries(clickhouse-odbc-bridge PRIVATE ${lib})" >> \
	...
	if use static-libs; then
		echo 'install(TARGETS zlibstatic)' >> contrib/zlib-ng/CMakeLists.txt
	else
		echo 'install(TARGETS zlib)' >> contrib/zlib-ng/CMakeLists.txt
	fi
	...
	if use json; then
		cmake_config_checks+=(
			"USE_RAPIDJSON	!cpu_flags_x86_avx2"
			"USE_SIMDJSON	cpu_flags_x86_avx2"
		)
	else
		cmake_config_checks+=( "USE_RAPIDJSON json" "USE_SIMDJSON json" )
	fi
	cmake_config_checks+=(
...
src_configure() {
	...
	if use static && [[ $(tc-getCC) == gcc ]]; then
		append-cxxflags -lgcc_s
	fi
	if use static || use static-libs; then
	...
	mycmakeargs+=(
		-D"BUILD_SHARED_LIBS"="$(usex static-libs no yes)"
		...
		-D"ENABLE_RAPIDJSON"="$(usex json $(usex '!cpu_flags_x86_avx2') no)"
		-D"USE_SIMDJSON"="$(usex json $(usex 'cpu_flags_x86_avx2') no)"
	)
	if use json && use '!cpu_flags_x86_avx2'; then
		mycmakeargs+=( -D"USE_INTERNAL_RAPIDJSON_LIBRARY"="yes" )
	fi
	...
	if use llvm; then
		mycmakeargs+=( -D"LLVM_VERSION"="$("$(get_llvm_prefix -b)"/bin/llvm-config --version)" )
	fi
...
src_install() {
	...
	if use static-libs; then
		for file in $(find "${BUILD_DIR}" -name 'lib*.a' || die "No static libraries found"); do
			dolib.a "${file}"
		done

		mkdir -p "${D}/usr/$(get_libdir)/${PN}"
		if use static; then
			mv -v ${D}/usr/{"$(get_libdir)","$(get_libdir)/${PN}"}/libre2.a || die "Failed to move libre2.a"
		fi
	fi
	...
	for file in "${D}/usr/bin/"*; do
		...
		if [[ ! "${file}" == *bin/clickhouse* ]] && [[ "${file}" == */bin/* ]]; then
			mkdir -p "${D}/usr/libexec/clickhouse/"
			mv ${file} "${D}/usr/libexec/clickhouse/" || die "can't move to libexec"
		fi
	done
