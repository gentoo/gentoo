...
inherit check-reqs cmake edo flag-o-matic linux-info multiprocessing prefix toolchain-funcs verify-sig
...
# Patch version
PATCH_SET=(
	https://github.com/parona-source/mysql-server/releases/download/mysql-8.4.0-patches-01/mysql-8.4.0-patches-01.tar.xz
)

DESCRIPTION="Fast, multi-threaded, multi-user SQL database server"
HOMEPAGE="https://www.mysql.com/"
SRC_URI="
	https://cdn.mysql.com/Downloads/MySQL-$(ver_cut 1-2)/mysql-${MY_PV}.tar.gz
	verify-sig? (
		https://cdn.mysql.com/Downloads/MySQL-$(ver_cut 1-2)/mysql-${MY_PV}.tar.gz.asc
	)
	${PATCH_SET[@]}
"
...
BDEPEND="
	...
	verify-sig? (
		~sec-keys/openpgp-keys-mysql-20231023
	)
"
...
src_compile() {
	...
	if use doc; then
		...
		local NUM_PROC_THREADS=$(makeopts_jobs)
		local DOT_NUM_THREADS=$(makeopts_jobs)
		local PLANTUML_JAR_PATH="${EPREFIX}/usr/share/plantuml/lib/plantuml.jar"
			...
			if [[ -n "${MYSQL_ROOT_PASSWORD}" ]] ; then
				if [[ ${MYSQL_ROOT_PASSWORD} == *$'\n'* ]] ; then
					ewarn "Ignoring password from '${tmp_mysqld_password_source}' section due to newline character \
						(do you have multiple password options set?)!"
					MYSQL_ROOT_PASSWORD=
					continue
				fi
