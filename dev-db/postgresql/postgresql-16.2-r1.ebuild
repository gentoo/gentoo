...
IUSE="debug doc +icu kerberos ldap libedit llvm +lz4 nls pam perl python
	  +readline selinux +server systemd ssl static-libs tcl uuid xml
	  zlib +zstd"

REQUIRED_USE="python? ( ${PYTHON_REQUIRED_USE} ) ?? ( libedit readline )"
...
src_configure() {
	...
	if use libedit; then
		rl_config="--with-readline --with-libedit-preferred"
	elif use readline; then
		rl_config="--with-readline"
	else
		rl_config="--without-readline"
	fi
