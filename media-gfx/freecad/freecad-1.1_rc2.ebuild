...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="
		https://github.com/${MY_PN}/${MY_PN}/releases/download/1.1rc2/freecad_source_1.1rc2.tar.gz -> ${P}.tar.gz
"
	KEYWORDS="~amd64"
	S="${WORKDIR}"
fi
...
PATCHES=(
	"${FILESDIR}"/${PN}-1.1_rc2-Gentoo-specific-don-t-check-vcs.patch
	"${FILESDIR}"/${PN}-1.1_rc2-tests-src-Qt-only-build-test-for-BUILD_GUI-ON.patch
