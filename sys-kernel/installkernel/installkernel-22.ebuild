...
IUSE="dracut grub systemd-kernel-install uki ukify"
...
src_install() {
	...
	if use systemd-kernel-install; then
	...
	elif use systemd-kernel-install; then
...
pkg_postinst() {
	...
	if [[ -n "${REPLACING_VERSIONS}" ]] && ver_test "${REPLACING_VERSIONS}" -lt 14; then
		...
		elog "kernel-install is controlled with the systemd-kernel-install"
		elog "USE flag or the SYSTEMD_KERNEL_INSTALL environment variable."
		elog
	...
	if [[ -z "${REPLACING_VERSIONS}" ]] || ver_test "${REPLACING_VERSIONS}" -lt 20; then
		if has_version "sys-boot/grub" && ! use grub; then
			...
			if use systemd-kernel-install; then
				elog "And to automatically have systemd's kernel-install use the"
				elog "correct /boot layout"
			fi
		fi
		if ( has_version "sys-apps/systemd[boot]" ||
			has_version "sys-apps/systemd-utils[boot]" ) &&
			! use systemd-kernel-install; then
				elog "systemd-boot is installed but the systemd-kernel-install USE flag"
