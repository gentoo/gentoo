...
inherit autotools cmake-utils eutils flag-o-matic java-pkg-opt-2 systemd xdg-utils

XSERVER_VERSION="1.20.6"

DESCRIPTION="Remote desktop viewer display system"
HOMEPAGE="https://tigervnc.org https://github.com/TigerVNC/tigervnc"
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-1.9.0-055_xstartup.patch"
)
...
src_prepare() {
	...
	if use server ; then
		...
		mkdir -p "${BUILD_DIR}/unix" || die
		cp -r "${S}"/unix/xserver "${BUILD_DIR}"/unix || die
		cp -r "${WORKDIR}"/xorg-server-${XSERVER_VERSION}/. "${BUILD_DIR}"/unix/xserver || die
		cd "${BUILD_DIR}"/unix/xserver || die
...
src_compile() {
	...
	if use server ; then
		...
		for d in "${subdirs[@]}"; do
			emake -C "${BUILD_DIR}"/unix/xserver/"${d}" TIGERVNC_SRCDIR="${S}" TIGERVNC_BUILDDIR="${BUILD_DIR}"
		done
	fi
}
...
src_install() {
	...
	if use server ; then
		emake -C "${BUILD_DIR}"/unix/xserver/hw/vnc DESTDIR="${D}" install
