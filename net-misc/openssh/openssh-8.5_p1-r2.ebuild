...
IUSE="abi_mips_n32 audit bindist debug hpn kerberos kernel_linux ldns libedit libressl livecd pam +pie +scp sctp security-key selinux +ssl static systemd test X X509 xmss"
...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-8.5_p1-systemd.patch
...
src_install() {
	...
	systemd_newunit "${FILESDIR}"/sshd_at_new.service 'sshd@.service'
	systemd_newunit "${FILESDIR}"/sshd_new.service 'sshd.service'
	systemd_newunit "${FILESDIR}"/sshd-keygen.target 'sshd-keygen.target'
	systemd_newunit "${FILESDIR}"/sshd-keygen_at.service 'sshd-keygen@.service'

	exeinto /usr/libexec
	doexe "${FILESDIR}"/sshd-keygen
}
