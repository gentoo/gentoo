...
DEPEND="
	${CDEPEND}
	caps? ( acct-user/ntp acct-group/ntp )
...
src_prepare() {
	...
	sed -i \
        ...
        cp "${FILESDIR}"/chronyd.conf-r3 "$T"/chronyd.conf
...
pkg_preinst() {
	if use caps && has_version net-misc/chrony[-caps]; then
		...
		elog "The safest thing to do is reboot to use chronyc, but you may chown manually."
