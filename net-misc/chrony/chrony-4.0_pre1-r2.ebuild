...
src_install() {
	...
	if use caps; then
		...
		mkdir -pv "${ED}"/var/lib/chrony || die
...
pkg_postinst() {
	...
	if use caps && ! use prefix; then
		# Permission changes on the livefs are not ideal.
		# Let's make sure they're necessary.
		# Versions with incorrect perms: <=chrony-3.5-r3, <=4.0_pre1-r1
		local ver

		for ver in ${REPLACING_VERSIONS}; do
			if ver_test ${ver} -le 4.0_pre1-r1; then
				ewarn "Adjusting permissions to ntp:ntp on /var/{lib,log}chrony/ for USE=caps"
				chown ntp:ntp /var/lib/chrony || die

				if [[ -f /var/lib/chrony/drift ]]; then
					chown ntp:ntp /var/lib/chrony/drift || die
				fi

				chown -R ntp:ntp /var/log/chrony || die
			fi
		done
	fi
}
