...
src_prepare() {
	...
	sed -e "s:'doc/kea':'doc/${P}':" \
		-i meson.build || die
	sed -e "s:'share/doc/kea':'share/doc/${P}':" \
		-i doc/meson.build || die
	sed -e "s:'doc/kea':'doc/${P}':" \
		-i doc/sphinx/meson.build || die
	sed -e "s:share/doc/kea/:share/doc/${P}/:" \
		-i doc/sphinx/arm/install.rst || die
	sed -e "s:share/doc/kea/examples:share/doc/${P}/examples:" \
...
src_install() {
	...
	# Tidy up
	rm -r "${ED}"/usr/share/kea/meson-info
	if use !mysql; then
		rm -r "${ED}"/usr/share/kea/scripts/mysql
	fi
	if use !postgres; then
		rm -r "${ED}"/usr/share/kea/scripts/pgsql
	fi
