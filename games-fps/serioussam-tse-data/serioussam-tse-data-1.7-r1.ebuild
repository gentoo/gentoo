...
src_install() {
	...
	mv "${D}/${dir}"/Scripts/PersistentSymbols.ini "${WORKDIR}" \
		|| die "failed to moved PersistentSymbols.ini"

	rm -rf \
		"${D}/${dir}"/{Bin,Controls,Data,Demos,Mods,Players,Scripts,Locales} \
		|| die "failed to remove directories"
	rm -rf \
		"${D}/${dir}"/{VirtualTrees,ModEXT.txt,Help/ShellSymbols.txt} \
		|| die "failed to remove directories"
	mkdir "${D}/${dir}/Scripts" || die "failed create Scripts dir"
	mv "${WORKDIR}"/PersistentSymbols.ini "${D}/${dir}/Scripts" \
		|| die "failed to moved PersistentSymbols.ini"

	# Remove useless Windows files
	rm -f "${D}/${dir}"/{*.exe,*.ex_,*.bmp,*.inx,*.hdr,*.bin,*.cab,*.ini,*.log} \
		|| die "Failed to remove windows cruft"

	# Ensure that file datestamps from the CD are sane
	find "${D}/${dir}"/Levels -exec touch -d '09 May 2020 14:00' '{}' \; \
		|| die "touch failed"
}
