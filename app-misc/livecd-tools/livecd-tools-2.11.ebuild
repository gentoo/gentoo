...
if [[ ${PV} == "9999" ]]; then
	...
	SRC_URI="
		https://gitweb.gentoo.org/proj/livecd-tools.git/snapshot/${P}.tar.bz2
		https://raw.githubusercontent.com/gentoo/livecd-tools/master/kernel/livecd-amd64.config"
...
src_install() {
	...
	if use dist-kernel; then
		...
		case ${ARCH} in
			amd64)
				doins "${DISTDIR}/livecd-amd64.config" || die
			;;
			*)
				die "RelEng currently does not support ${ARCH} as a dist kernel option"
			;;
		esac
	fi
}
