...
#
# Also it fails with >=jdk-23 so we limit the max version #936549
...
PATCHES=(
	"${FILESDIR}"/${PN}-3.2.0.0-disable-SWTUpdateChecker.patch
	"${FILESDIR}"/${PN}-3.2.0.0-disable-shared-plugins.patch
	"${FILESDIR}"/${PN}-3.5.0.0-Entities.javadoc.patch
	"${FILESDIR}"/${PN}-3.6.0.0-disable-PluginUpdatePlugin.patch
	"${FILESDIR}"/${PN}-3.6.0.0-fix-ambiguous.patch
)
...
src_compile() {
	...
	ejavac -d target/classes \
		-cp "$(java-pkg_getjars commons-cli):$(java-pkg_getjars swt-${SWT_SLOT})" \
