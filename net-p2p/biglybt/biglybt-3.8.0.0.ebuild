...
src_prepare() {
	...
	# Clean up the repo tarball
	find core/src -type f -name '.editorconfig' -delete
	...
	cp -r --reflink=always core/{src,resources} || die
	rm -r core/resources/META-INF || die "deleting META-INF"
	find core/resources -type f -name '*.java' -delete || die "core deleting *.java"

	cp -r --reflink=always uis/{src,resources} || die
	find uis/resources -type f -name '*.java' -delete || die "uis deleting *.java"
}
...
src_install() {
	...
	make_desktop_entry "${PN}" BiglyBT "${PN}" "Network;FileTransfer;P2P;GTK;Java"
