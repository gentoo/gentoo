...
# [ ] go through src_install() and re-check all these fowner calls
#       -> still necessary with GLEP81?
# [ ] update the fping-check: "if use server || use proxy ; then"
#       -> fping now has USE="+filecaps", so there is a sane default
#          to allow users using fping
#       -> proposal: check if filecaps or suid are active and tell the user
#          that fping needs one of these *if* neither of both is set
# [ ] add agent2 which is written in golang
...
RDEPEND="${COMMON_DEPEND}
	agent? ( acct-user/zabbix-agent acct-group/zabbix-agent )
	java? ( >=virtual/jre-1.8:* )
	mysql? ( virtual/mysql )
	proxy? (
		acct-user/zabbix-proxy acct-group/zabbix-proxy
		net-analyzer/fping
	)
	server? (
		acct-user/zabbix-server acct-group/zabbix-server
...
DOCS=( dodoc README INSTALL NEWS ChangeLog
