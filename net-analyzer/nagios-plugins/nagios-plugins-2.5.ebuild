...
IUSE="ipv6 ldap mysql nagios-dns nagios-ping nagios-game nls postgres samba selinux snmp ssh +ssl suid rpc"
...
PATCHES=(
	"${FILESDIR}/${PN}-2.4.12-postgresql-detection.patch"
	"${FILESDIR}/${PN}-2.4.12-snmpgetnext.patch"
	"${FILESDIR}/${P}-check-icmp-segfault.patch"
)
...
src_prepare() {
	...
	# Don't warn about needing to be run as root; hopefully you
	# have USE=filecaps on and that's not true.
	sed -e '/^[ \t]*np_warn_if_not_root();$/d' \
		-i plugins-root/check_{dhcp,icmp}.c || die
