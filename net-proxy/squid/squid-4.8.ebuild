...
COMMON_DEPEND="caps? ( sys-libs/libcap )
	pam? ( virtual/pam )
	...
	ssl? (
		...
		dev-libs/nettle net-libs/gnutls )
...
pkg_postinst() {
	...
	elog "'squidclient mgr:' commands such as 'squidclient mgr:info'."
	if [ ${#r} -gt 0 ]; then
		elog "You are using a release with the official ${r} patch! Make"
		elog "sure you mention that, or send the output of 'squidclient mgr:info'"
		elog "when asking for support."
	fi
}
