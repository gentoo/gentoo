...
src_prepare() {
	epatch "${FILESDIR}"/${P}-Makefile.in.patch || die
#	epatch "${FILESDIR}"/${P}-Makefile.am.patch || die
	epatch "${FILESDIR}"/${P}-${PN}.conf.patch || die
	epatch "${FILESDIR}"/${P}-${PN}.h.patch || die
	epatch "${FILESDIR}"/${P}-${PN}.c.patch || die
	epatch "${FILESDIR}"/${P}-clwarn.cgi.patch || die
#	epatch "${FILESDIR}"/${P}-configure.in.patch || die
