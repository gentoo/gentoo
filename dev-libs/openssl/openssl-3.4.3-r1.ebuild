...
IUSE="+asm cpu_flags_x86_sse2 doc fips ktls +quic rfc3779 sctp static-libs test tls-compression vanilla weak-ssl-ciphers"
RESTRICT="!test? ( test )"
...
pkg_setup() {
	...
	if use doc && ! $(multilib_is_native_abi) ; then
		die "USE=doc only supported on the profile native ABI"
	fi
}
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		...
		emake DESTDIR="${D}" DOCDIR='$(INSTALLTOP)'/share/doc/${PF} -j1 $(usex doc install_docs "")
	fi
