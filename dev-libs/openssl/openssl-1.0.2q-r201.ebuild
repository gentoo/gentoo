...
pkg_pretend() {
	if has_version "dev-libs/openssl:1.0.0[versioned]" && ! use versioned; then
		eerror "You are disabling versioned symbols because of that you have to rebuild all packages depending on OpenSSL!"
	fi
}
...
src_prepare() {
	...
	if use versioned ; then
		eapply "${FILESDIR}"/openssl-1.0.2g-version-script.patch
	fi
