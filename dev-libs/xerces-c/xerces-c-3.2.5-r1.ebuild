...
src_configure() {
	...
	local netaccessor=$(usex elibc_Darwin fcurl socket)
	use curl && netaccessor="curl"

	local msgloader="inmemory"
	use iconv && msgloader="iconv"
	use icu && msgloader="icu"

	# REVIEW: seems like this won't work on musl
	local transcoder=$(usex elibc_Darwin macosunicodeconverter gnuiconv)
