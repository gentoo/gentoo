...
SRC_URI+=" https://raw.githubusercontent.com/vim/vim/v${XXD_VERSION}/src/xxd/xxd.c ->
	xxd-${XXD_VERSION}.c"
...
src_unpack() {
	[[ ${PV} != *9999 ]] && unpack ${P}.tar.gz || git-r3_src_unpack
	cp "${DISTDIR}"/xxd-${XXD_VERSION}.c "${S}"/xxd.c || die
}
...
src_compile() {
	# Build xxd
	mkdir "${BUILD_DIR}"/xxd || die
	$(tc-getCC) ${CFLAGS} ${LDFLAGS} -DUNIX -o "${BUILD_DIR}"/xxd/xxd "${S}"/xxd.c || die
	chmod +x "${BUILD_DIR}"/xxd/xxd || die
	export PATH="${BUILD_DIR}/xxd:${PATH}"
