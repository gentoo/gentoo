...
pkg_pretend() {
	if tc-is-gcc; then
		...
		then
			eerror "Boost is now build in C++11 mode, and therefore requires at least GCC 4.9"
			die "Active GCC too old."
		fi

		if [[ $(gcc-major-version) -eq 4 && $(gcc-minor-version) -eq 9 ]]; then
			ewarn "You are compiling boost with GCC 4.9. While this configuration is likely"
			ewarn "to work, it is not recommended, due to the ABI break involved when moving"
			ewarn "to a more modern version of GCC. Tread carefully."
		fi
	fi
}
...
src_configure() {
	...
	# Switch the boost-dependent ecosystem to C++11 globally
