...
inherit autotools dot-a flag-o-matic libtool multilib multilib-minimal verify-sig
...
"
BDEPEND="
	verify-sig? ( sec-keys/openpgp-keys-guillemjover )
	${AUTOTOOLS_DEPEND}
"

# bug #925663: ${P}-multilib-time_abi.patch edits configure.ac/Makefile.am, so after
# default applies it the tarball's configure and Makefile.in are stale; eautoreconf in
# src_prepare refreshes them. When fixed upstream, drop the patch, eautoreconf,
# ${AUTOTOOLS_DEPEND}, and replace autotools with libtool in inherit as before.
PATCHES=(
	"${FILESDIR}/libbsd-install-per-libdir-time_abi.h-for-multilib-ti.patch"
)
