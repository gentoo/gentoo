...
multilib_src_configure() {
	...
	if multilib_native_use introspection; then
		...
		pushd ${INTROSPECTION_SOURCE_DIR}

		meson_src_configure
		meson_src_compile
		# Install to the portage temp directory so that pkgconfig relative paths resolve correctly
		# The install directory will get wiped by portage before the actual install stage
		...
		export PKG_CONFIG_LIBDIR="${D}/usr/$(get_libdir)/pkgconfig:${INTROSPECTION_BUILD_DIR}/meson-private"
		...
		export PYTHONPATH="${D}/usr/$(get_libdir)/gobject-introspection:${PYTHONPATH}"
	fi
