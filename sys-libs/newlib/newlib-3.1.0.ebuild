...
if [[ ${PV} == "9999" ]] ; then
	...
	if [[ ${PV} != *.201[5-9]???? ]] ; then
		KEYWORDS="-* ~arm ~hppa ~m68k ~mips ~ppc ~ppc64 ~riscv ~sh ~sparc ~x86"
	fi
fi
...
src_configure() {
	...
	export "CFLAGS_FOR_TARGET=${CFLAGS_ORIG} ${CFLAGS_FULL} -U_FORTIFY_SOURCE"
	export "CCASFLAGS=${CCASFLAGS_ORIG} ${CFLAGS_FULL} -U_FORTIFY_SOURCE"
	ECONF_SOURCE=${S} \
	econf \
