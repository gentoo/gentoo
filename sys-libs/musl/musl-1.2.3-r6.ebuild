...
SLOT="0"
IUSE="crypt custom-cflags headers-only"
...
src_configure() {
	...
	if ! use custom-cflags; then
		...
		# set, some are known to break Musl so filtered to aviod issues.
		replace-flags -Ofast -O3
		# LTO is known not to work on the Musl package so removed, bug 877343
		filter-lto
	fi
