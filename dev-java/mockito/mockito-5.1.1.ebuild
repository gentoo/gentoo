...
MAVEN_ID="org.mockito:mockito-core:5.1.1"
...
src_compile() {
	...
	unzip "${S}"/mockito.jar \
		org/mockito/internal/creation/bytebuddy/inject/MockMethodDispatcher.class || die
	zip -d "${S}"/mockito.jar \
		org/mockito/internal/creation/bytebuddy/inject/MockMethodDispatcher.class || die
	mv org/mockito/internal/creation/bytebuddy/inject/MockMethodDispatcher.{class,raw} || die
	jar uf "${S}"/mockito.jar \
