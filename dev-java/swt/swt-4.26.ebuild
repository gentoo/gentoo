...
MY_PV="${PV/_rc/RC}"
MY_DMF="https://download.eclipse.org/eclipse/downloads/drops4/R-${MY_PV}-202211231800"
MY_P="${PN}-${MY_PV}"
...
src_prepare() {
	...
	unzip swt.jar 'org/eclipse/swt/internal/gtk/*.css' -d "${JAVA_RESOURCE_DIRS}" || die
	java-pkg_clean
	mkdir "${JAVA_SRC_DIR}" || die "mkdir failed"
	mv org "${JAVA_SRC_DIR}" || die "moving java sources failed"
	cp -r "${JAVA_SRC_DIR}" "${JAVA_RESOURCE_DIRS}" || die "copying resources failed"
	find "${JAVA_RESOURCE_DIRS}" -type f -name '*.java' -exec rm -rf {} + \
		|| die "deleting classes failed"
