...
AT_x86="jdk-${MY_PV}_linux-i586_bin.tar.gz"
...
DEMOS_amd64="jdk-${MY_PV}_linux-x64_bin-demos.tar.gz"
DEMOS_arm="jdk-${MY_PV}_linux-arm32_bin-vfp-hflt-demos.tar.gz"
DEMOS_arm64="jdk-${MY_PV}_linux-arm64-vfp-hflt-demos.tar.gz"
DEMOS_x86="jdk-${MY_PV}_linux-i586_bin-demos.tar.gz"
DEMOS_x64_solaris="jdk-${MY_PV}_solaris-amd64_bin-demos.tar.gz"
DEMOS_sparc64_solaris="jdk-${MY_PV}_solaris-sparcv9_bin-demos.tar.gz"
DEMOS_x64_macos="jdk-${MY_PV}_macosx-x86_64_bin-demos.zip"
...
src_unpack() {
	if use x64-macos ; then
		...
		mkdir dmgmount || die
		hdiutil attach "${DISTDIR}"/jdk-${MY_PV}_osx-x64_bin.dmg \
...
src_install() {
	...
	dodir "${dest}"
	cp -pPR bin include lib jmods "${ddest}" || die
	...
	java-vm_install-env "${FILESDIR}"/${PN}-$(get_version_component_range 1-2 ${PV}).env.sh
	java-vm_revdep-mask
