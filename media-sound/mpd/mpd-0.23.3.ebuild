...
RDEPEND="
	...
	amd64? ( npupnp? ( net-libs/libnpupnp:0 ) )
	x86? ( npupnp? ( net-libs/libnpupnp:0 ) )
...
src_configure() {
	...
	if use upnp; then
		emesonargs+=( -Dupnp=pupnp )
	elif use npupnp; then
		emesonargs+=( -Dupnp=npupnp )
	else
		emesonargs+=( -Dupnp=disabled )
	fi
