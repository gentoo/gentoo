...
SLOT="0/${PV%%.*}"
# unkeyworded for testing linux-mod-r1
...
src_compile() {
	...
	if use driver; then
		...
		local modlistargs=kernel/drivers/video:kernel
		if use kernel-open; then
			modlistargs+=-module-source:kernel-module-source/kernel-open

			# nvidia uses flags when the building the "blob" and fails with
			# lto, also need strip in case of a different kernel toolchain
			filter-lto
