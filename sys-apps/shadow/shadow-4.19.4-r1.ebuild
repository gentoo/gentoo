...
inherit fcaps libtool pam user-info verify-sig
...
pkg_postinst() {
	...
	if [[ -n ${ROOT} ]]; then
		...
		if [[ -n ${group} ]]; then
			chgrp "${group}" "${EROOT}"/usr/bin/{chage,expiry} || die
			chmod g+s "${EROOT}"/usr/bin/{chage,expiry} || die
		fi
	fi
	...
	if [[ ! -f "${EROOT}"/etc/gshadow ]] ; then
		if grpck -r -R "${EROOT:-/}" 2>/dev/null ; then
			grpconv -R "${EROOT:-/}"
			if [[ -n ${group} ]]; then
				chgrp "${group}" "${EROOT}"/etc/gshadow || die
				chmod g+r "${EROOT}"/etc/gshadow || die
			fi
		else
