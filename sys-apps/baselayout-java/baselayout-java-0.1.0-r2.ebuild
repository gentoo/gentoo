...
src_install() {
	...
	newexe - java-cacerts <<-_EOF_
		#!/bin/sh
		trust extract --overwrite --format=java-cacerts --filter=ca-anchors --purpose server-auth "${EPREFIX}/etc/ssl/certs/java/cacerts"

		# Java 9 or higher uses PKCS#12 as default format (#712290)
		trust extract --overwrite --format=pem-bundle --filter=ca-anchors --purpose server-auth "${EPREFIX}/etc/ssl/certs/java/cacerts-pkcs12"
	_EOF_
}
