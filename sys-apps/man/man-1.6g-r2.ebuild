...
src_prepare() {
	...
	eapply -p1 "${FILESDIR}"/man-1.6f-unicode.patch #146315
	eapply -p1 "${FILESDIR}"/man-1.6c-cut-duplicate-manpaths.patch
	eapply -p1 "${FILESDIR}"/man-1.5m2-apropos.patch
	eapply -p1 "${FILESDIR}"/man-1.6g-fbsd.patch #138123
	eapply -p0 "${FILESDIR}"/man-1.6e-headers.patch
	eapply -p1 "${FILESDIR}"/man-1.6f-so-search-2.patch
	eapply -p0 "${FILESDIR}"/man-1.6g-compress.patch #205147
	eapply -p1 "${FILESDIR}"/man-1.6f-parallel-build.patch #207148 #258916
	eapply -p1 "${FILESDIR}"/man-1.6g-xz.patch #302380
	eapply -p1 "${FILESDIR}"/man-1.6f-makewhatis-compression-cleanup.patch #331979
	eapply -p1 "${FILESDIR}"/man-1.6g-echo-escape.patch #523874
	eapply -p1 "${FILESDIR}"/man-1.6g-hurd.patch
