...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-unmatched-entries.patch
	epatch "${FILESDIR}"/${PN}-treat-compressed-logfiles-always-as-arc.patch
	epatch "${FILESDIR}"/${PN}-use-custom-pattern-for-applystd.patch
	epatch "${FILESDIR}"/${PN}-revert-fail2ban-service-change.patch
	epatch "${FILESDIR}"/${PN}-support-openspf.net-in-postfix.patch
	epatch "${FILESDIR}"/${PN}-perl-warning-redundant-argument.patch
	epatch "${FILESDIR}"/${PN}-dovecot-ignore-debug.patch
	eapply_user
}
...
src_install() {
	...
	newexe "${FILESDIR}"/${PN} 00-logwatch
