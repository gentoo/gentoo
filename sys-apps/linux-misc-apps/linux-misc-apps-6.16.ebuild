...
if [[ ${PV/_rc} != ${PV} ]]; then
	...
	SRC_URI="
		https://www.kernel.org/pub/linux/kernel/v${PV%%.*}.x/testing/${LINUX_PATCH}
		https://www.kernel.org/pub/linux/kernel/v${PV%%.*}.x/testing/v${PATCH_VERSION}/${LINUX_PATCH}
	"
elif [[ $(_get_version_component_count) == 4 ]]; then
	# stable-release series
	LINUX_VER=$(ver_cut 1-3)
	LINUX_PATCH=patch-${PV}.xz
	SRC_URI="https://www.kernel.org/pub/linux/kernel/v${PV%%.*}.x/${LINUX_PATCH}"
else
	LINUX_VER=${PV}
fi

LINUX_SOURCES=linux-${LINUX_VER}.tar.xz
SRC_URI+=" https://www.kernel.org/pub/linux/kernel/v${PV%%.*}.x/${LINUX_SOURCES}"
