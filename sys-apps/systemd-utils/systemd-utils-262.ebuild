...
multilib_src_compile() {
	if multilib_is_native_abi; then
		...
		local args=( libudev src/libudev/libudev.pc )
		use test && args+=( test-libudev test-libudev-sym test-udev-device-thread )
...
multilib_src_test() {
	...
	if multilib_is_native_abi; then
		...
		use ukify && args+=( --suite ukify )
	fi
	if [[ ${#args[@]} -gt 0 ]]; then
		meson_src_test --no-rebuild "${args[@]}"
	fi
}
