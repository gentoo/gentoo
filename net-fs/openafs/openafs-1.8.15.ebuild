...
PATCHES=(
		...
		"${FILESDIR}"/0017-docbook2pdf.patch
		"${FILESDIR}"/0010-libperl.patch
		"${FILESDIR}"/0011-xbsa.patch
		"${FILESDIR}"/0012-xml-dtd.patch
		"${FILESDIR}"/0016-kernel-cc-ld.patch
		"${FILESDIR}"/0018-openafs-1.8.15-linux-6.17.patch
)
...
src_install() {
		...
		systemd_newunit "${SYSTEMDDIR}"/01-openafs-client.service openafs-client.service
		systemd_dounit "${SYSTEMDDIR}"/openafs-server.service

		systemd_install_serviced "${SYSTEMDDIR}"/openafs-server.service.conf

		exeinto /usr/libexec/openafs
		doexe "${SYSTEMDDIR}"/openafs-client-systemd-helper.sh
...
pkg_postinst() {
		...
		elog
		elog "openafs-1.8.15 introduced a helper script for the systemd"
...
pkg_config() {
		...
		grep -q "^Environment=\"AFSD_CACHE_ARGS=" "${SERVICED_FILE}" && \
				sed -i "s/^Environment=\"AFSD_CACHE_ARGS=.*/Environment=\"AFSD_CACHE_ARGS=${AFSD_CACHE_ARGS}\"/" \
					"${SERVICED_FILE}" || \
