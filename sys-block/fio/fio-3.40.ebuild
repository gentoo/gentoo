...
IUSE="aio curl glusterfs gnuplot gtk io-uring isa-l iscsi lex nbd nfs numa pandas python rbd rdma static tcmalloc test valgrind zbc zlib"
...
BDEPEND+="
	lex? ( app-alternatives/lex app-alternatives/yacc )"
...
src_prepare() {
	...
	# >net-fs/libnfs-6 implements RPC-with-TLS support which requires gnutls.
	# Remove gnutls requirement for <net-fs/libnfs-6.
	if has_version '<net-fs/libnfs-6'; then
		sed -i -E -e "/libnfs_(cflags|libs)=/s: gnutls::" configure || die
	fi
}
...
src_configure() {
	...
	# TODO: libhdfs
	# TODO: ime_native - not packaged in Gentoo
	# rbd option does not work as expected:
	...
	OPTS=(
		...
		$(usex isa-l '' '--disable-isal')
		$(usev iscsi '--enable-libiscsi')
		$(use_enable lex)
		$(usev nbd '--enable-libnbd')
		$(use_enable nfs libnfs)
		$(usex numa '' '--disable-numa')
		$(usex rbd '' '--disable-rados')
		$(usex rbd '' '--disable-rbd')
		$(usex rdma '' '--disable-rdma')
		$(usex static '--build-static' '')
		$(usex tcmalloc '' '--disable-tcmalloc')
		$(usex zbc '' '--disable-libzbc')
	)
	set -- \
	# not autoconf
	./configure "${OPTS[@]}"
