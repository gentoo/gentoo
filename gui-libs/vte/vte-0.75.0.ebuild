...
IUSE="+crypt debug gtk-doc +icu +introspection prompt sixel systemd +vala vanilla"
...
SRC_URI="${SRC_URI} prompt? ( https://dev.gentoo.org/~mattst88/distfiles/${PN}-0.75.0-prompt-patchset.patch.xz )"
...
src_prepare() {
	...
	if use prompt; then
		eapply "${WORKDIR}"/${PN}-0.75.0-prompt-patchset.patch
	fi
