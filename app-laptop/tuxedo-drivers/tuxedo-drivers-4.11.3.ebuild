...
pkg_setup() {
	...
	local ERROR_LEDS_CLASS_MULTICOLOR="CONFIG_LEDS_CLASS_MULTICOLOR: is not set but needed for keyboard backlight"
	# Better descriptions of why these are needed should be added at a later date
	local ERROR_ACPI_WMI="CONFIG_ACPI_WMI: is not set but required for tuxedo-drivers"
	local ERROR_INPUT_SPARSEKMAP="CONFIG_INPUT_SPARSEKMAP: is not set but required for tuxedo-drivers"
	local ERROR_IIOP="CONFIG_IIO: is not set but required for tuxedo-drivers"
