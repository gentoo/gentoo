...
# List of all features sets we support.
IUSE="${IUSE} vim_features_tiny vim_features_small vim_features_normal vim_features_big"
...
src_prepare() {
	...
	if [[ -d "${S}"/src/po ]]; then
		sed \
			-i '/-S check.vim/s,..VIM.,ln -s $(VIM) testvim \; ./testvim -X,' \
...
src_configure() {
	...
	if use minimal; then
		...
		# Features sets management.
		local vim_featureset="huge"
		use vim_features_big    && vim_featureset="big"
		use vim_features_normal && vim_featureset="normal"
		use vim_features_small  && vim_featureset="small"
		use vim_features_tiny   && vim_featureset="tiny"

		myconf=(
			--with-features="${vim_featureset}"
			--enable-multibyte
			$(use_enable acl)
			$(use_enable cscope)
			$(use_enable gpm)
		...
		if ! use cscope && [[ "${vim_featuress}" == "huge" ]]; then
...
src_install() {
	...
	rm -v "${ED}"/usr/share/man/man1/{ex,view}.1 || die
}
